...
<section style="padding-bottom: 300px" id ="boardSc">
...
<h3 class="bestHeaderText">자유게시판 인기글</h3><div id="nextList"></div></header>
	<div class="bestBody">
		<ul id="bestList">
		</ul>	
	</div>
...
<div class="col-lg-3" id="sidemenu">
	...
	border-width: 2px 0px 2px 0px;">
		...
		<!-- /.col-lg-3 -->
		<div class="col-lg-9" id="listTable">
			<table class="table table-hover" style="background-color:white; border: 2px solid #000000; border-width: 2px 0px 0px 0px;">
				<thead>
					<tr>
						<th>번호</th>
						<th>작성자</th>
	...
	<div style="position: relative; top: 45px;">
		<div class="write">
			<c:if test="${!empty sessionScope.user}" >
				<ui><a href="/views/community/free_write"><button type="button" class="btn_blue write">글쓰기</button></a></ui>
			</c:if>
		</div>
		<div class="bottom-2">
			<div class="search row" id="search_row">
			 ...
			 <div  class="bottom_search fl clear" id="zz">
				 <div class="inner_search">
			 		  <input class="in_keyword" id="in_keyword" type="text" id="keyword" name="search_keyword" title="검색어 입력" value="">	 	 	
			 	 </div>  
	...
	$.ajax({	
		url : '/community/selectList',
				...
				success : function(res) {
					...
					document.querySelector("#freeList").innerHTML = html;
					var page = '';
	...
	$.ajax({
		...
		success : function(res){
			...
			+ userNum+'">'+col.guideTitle.substring(0,7)+'...</a></td>';
				...
				if(col.guideTitle=="게시글이 없습니다"){
					html +='<td>'+col.guideTitle+'</td>';
				}else{
					html += '<td><a href="/views/community/guideView?guideNum=' + col.guideNum
					+ '">'+col.guideTitle+'</td>';
					}				
...
function getBestList(cnt){
	...
	$.ajax({
		url:"/community/selectBestFreeList",
		data:{ cnt : cnt
			},
		success : function(res){
			var html ='';
			for(var idx in res.rList){
				var col = res.rList[idx];
				if(col.freeTitle=="인기글이 없습니다"){
					html +='<li class="bestlistText">'+col.freeTitle+'</li>';
				}else{
					if(col.freeTitle.length>=11){
						html +='<li class="bestlistText"><a href="/views/community/freeView?freeNum='+col.freeNum+'">'+col.freeTitle.substring(0,9)+'...</a></li>';							
						}else{
							if(col.guideTitle=="인기글이 없습니다"){
								html +='<li class="bestlistText">'+col.freeTitle+'</li>';
							}else{
								html +='<li class="bestlistText"><a href="/views/community/freeView?freeNum='+col.freeNum+'">'+col.freeTitle+'</a></li>';
								}				
						}
