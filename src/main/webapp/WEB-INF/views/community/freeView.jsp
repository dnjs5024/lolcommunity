...
<header>
	  <div class="writerinfo">
			...
			<a href="/views/community/freeUpdate?freeNum=${param.freeNum}"><button class="btn btn-outline-info">수정</button></a>
			<button onclick="freeBoardDelete(${sessionScope.user.userNum})" class="btn btn-outline-danger">삭제</button>
		</div>	
			<div class="fl" style="position:relative;bottom:40px; ">
				<span id='userNick'><em></em></span>
				<span id="ip">(223.33)</span>
				<span id="freeJoin" style="margin-right: 5px;"></span>					
				<span>조회</span>	<span id="freeCnt" style="margin-right: 2px;"></span>
	...
	<div class="inner clear">
		<div class="writing_view_box" style=" min-width: 100%;" >
			...
			<div style="overflow: hidden; min-width: 100%; min-height: 400px; vertical-align: top;">		
				...
				<div id="freeContent" style="float:left;"></div>
			</div>
		</div>
	</div>
</div>				
	  ...
	  <c:when test="${sessionScope.user ==null }">
		...
		<div class="card-footer" id="card-footer" style="min-width: 100%;">
			...
			<div class="btn_wrap" style="text-align: center;">
				...
				<a href="javascript:commonViewMore();" class="btn_more"  style=" padding-left: 650px; padding-right: 650px;">더보기</a>
...
<a href="/views/community/freeBoard"><button class="btn_blue concept" style="margin-top:20px; "><span>전체글</span></button></a>
</div>
...
<div class="container">
<div class="freeview-bottom" style="background-color: white;" style="margin:0px; padding:0px;">
...
function selectUserOne(){
	  ...
	  $.ajax({
			...
			success : function(res){
				...
				for(var idx in res){
					...
					tagList += "<button  onclick='selectTagOne("+res[idx].tagNum+")'class='btn btn-outline-success' data-toggle='modal' data-target='#exampleModal' type='button'>수정 / 삭제</button>";
					tagList += '</li>';
					tagList += '<hr/>';
  ...
  function insertGhostTag(){
	  ...
	  $.ajax({
		  url : '/community/insertTag',
		  type : 'POST',
		  data : params,
		  success : function(res){
			  if(res ==1){
					location.href='/views/community/freeView?freeNum='+freeNum;
				}
		  }
	  })
  ...
  function selectTagOne(cnt){
	  $.ajax({
		  ...
		  success : function(res){
			  ...
			  $('.modal-body > #modalTagNum').val(res.tagNum);
		  }
	  })
  }
  ...
  function selectOne(){
	$.ajax({
		url : '/community/selectOne',
		type : 'POST',
		data : param,
		success : function(res){
			document.getElementById('freeTitle').innerHTML=res.freeTitle;
			document.getElementById('userNick').innerHTML=res.userNick;
			document.getElementById('freeJoin').innerHTML=res.freeJoin;
			document.getElementById('freeContent').innerHTML=res.freeContent;
			document.getElementById('freeCnt').innerHTML=res.freeCnt;
			...
			if(res.freeFilepath !=null){
				$('#imgId').attr('src','/resources/freeBoardimg/'+res.freeFilepath);
				$('#filePath').val(res.freeFilepath);
			}	
			if(res.like){
...
function freeBoardDelete(cnt){
	var freeFilepath = $('#filePath').val();
	var userNum = cnt;
	var param = {
			freeNum:freeNum,
		...
		url : '/community/selectList',
				...
				success : function(res) {
					...
					for ( var idx in res.rList) {
						...
						html += '<td><a href="javascript:void(0)" onclick="freeUpdate('
								+ col.freeNum + ')">' + col.freeTitle+'  ['+col.tagCnt+']'
								+ '</a></td>';
						}else {
							html += '<td><a href="javascript:void(0)" onclick="freeUpdate('
								+ col.freeNum + ')">' + col.freeTitle;
								+ '</a></td>';
						}
						if (col.freeFilepath != null) {
							html += '<td><img src="/resources/freeBoardimg/'+col.freeFilepath+' "width="50px"></td>';
						} else {
							html += '<td></td>';
						}
...
function doSearch() {
	...
	var keyword = $('#keyword').val();
			...
			$.ajax({
				url : '/community/selectSearchList',
				data : {
					type : type,
					keyword : keyword
				},
				success : function(res) {
					...
					for ( var idx in res.rList) {
						...
						html += '<td><a href="javascript:void(0)" onclick="freeUpdate('
								...
								+ '</a></td>';
						if (col.freeFilepath != null) {
							html += '<td><img src="/resources/'+col.freeFilepath+'"width="50px"></td>';
						} else {
							html += '<td></td>';
						}
						html += '<td>' + col.freeJoin + '</td>';
