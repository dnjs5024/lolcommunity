...
<div class="col-sm-10" style="margin-top: 15px;">
		<input class="form-control input-sm" type="text" id="freeTitle" >
		<hr>
	</div>
	...
	<div class="col-sm-10">
	  ...
	  <textarea id="freeContent" ></textarea>
...
function selectOne(){
	$.ajax({
		...
		data :param,
		success : function(res){
			$('#freeTitle').val(res.freeTitle);
			$('#freeContent').summernote('code',res.freeContent);
...
function update(){
	var freeTitle = $('#freeTitle').val().trim();
	var freeContent = $('#freeContent').val().trim();
	
	
	if(freeTitle.length <= 1){
		alert('제목을 입력해주세요');
		$('#freeTitle').focus;
		return;
	}
	
	if(freeContent.length <= 1){
		alert('내용을 입력해주세요');
		$('#freeContent').focus;
		return;
	}
	var formData = new FormData($('#formData')[0]);
		...
		formData.append('freeNum',freeNum);
